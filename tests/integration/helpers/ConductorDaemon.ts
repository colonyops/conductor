...
export class ConductorDaemon {
  ...
  async start(env: TestEnv): Promise<void> {
    this.env = env;
    this.proc = Bun.spawn(
      ["bun", "src/index.ts", "start", "--config", env.configPath],
      {
        cwd: CONDUCTOR_ROOT,
        env: env.processEnv,
        stdout: "pipe",
  ...
  async waitForReady(timeoutMs = 20_000): Promise<void> {
    if (!this.env)
      throw new Error("start() must be called before waitForReady()");
    ...
    throw new Error(
      `Conductor did not start within ${timeoutMs}ms. Check log: ${logPath}`,
    );
  }

  /** Send a lifecycle signal for a session. */
  async signal(
    type: "activity" | "stop" | "stop:approval",
    sessionId: string,
  ): Promise<void> {
    if (!this.env) throw new Error("start() must be called before signal()");
    const proc = Bun.spawn(
      ["bun", "src/index.ts", "signal", type, "--session", sessionId],
      {
        cwd: CONDUCTOR_ROOT,
  ...
  async stop(timeoutMs = 35_000): Promise<number> {
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(new Error(`Conductor did not stop within ${timeoutMs}ms`)),
          timeoutMs,
        ),
      ),
    ]);
