...
const SLOW_INIT = join(
  import.meta.dir,
  "../fixtures/plugins/slow-init.plugin.ts",
);
const FAST_INIT = join(
  import.meta.dir,
  "../fixtures/plugins/fast-init.plugin.ts",
);
...
describe("06 — plugin init timeout", () => {
  ...
  it("logs that the slow plugin timed out", async () => {
    const entry = await pollLog(
      env.logPath,
      (e) =>
        e.msg === "Plugin init timed out" && e.data?.plugin === "Slow Init",
      5_000,
    );
  ...
  it("fast-init plugin still loads when slow-init times out", async () => {
    ...
    const loaded = parseLog(content).find(
      (e) => e.msg === "Plugin loaded" && e.data?.pluginId === "test-fast-init",
    );
  ...
  it("conductor reports pluginCount: 1 (only fast-init loaded)", async () => {
    const content = await Bun.file(env.logPath).text();
    const started = parseLog(content).find(
      (e) => e.msg === "Conductor started",
    );
